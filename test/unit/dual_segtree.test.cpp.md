...
data:
  ...
  bundledCode: "#line 1 \"test/unit/dual_segtree.test.cpp\"\n#define PROBLEM \"http://judge.u-aizu.ac.jp/onlinejudge/description.jsp?id=ITP1_1_A\"\
    ...
    \ F, F (*composition)(F, F), F (*id)(), bool is_commutative = false>\nstruct dual_segtree\
    \ {\npublic:\n    dual_segtree() {\n    }\n    dual_segtree(int n) {\n       \
    ...
    \    const int N = 30;\n    const int Q = 2000;\n    dual_segtree<F, composition,\
    \ id, is_commutative> seg(N);\n    std::vector<F> naive(N, id());\n    auto gen\
    \ = [&]() -> F {\n        if constexpr (is_commutative) {\n            return\
    ...
    \ = rng() % N;\n            int r = l + 1 + rng() % (N - l);\n            F f\
    \ = gen();\n            seg.apply(l, r, f);\n            for (int i = l; i < r;\
    \ i++) naive[i] = composition(f, naive[i]);\n        } else if (t == 1) {\n  \
    \          int p = rng() % N;\n            F v = gen();\n            seg.set(p,\
    \ v);\n            naive[p] = v;\n        } else {\n            int p = rng()\
  ...
  code: "#define PROBLEM \"http://judge.u-aizu.ac.jp/onlinejudge/description.jsp?id=ITP1_1_A\"\
    ...
    \ <bool is_commutative>\nvoid stress(std::mt19937 &rng) {\n    const int N = 30;\n\
    \    const int Q = 2000;\n    dual_segtree<F, composition, id, is_commutative>\
    \ seg(N);\n    std::vector<F> naive(N, id());\n    auto gen = [&]() -> F {\n \
  ...
  requiredBy: []
  timestamp: '2026-05-18 10:25:36+09:00'
